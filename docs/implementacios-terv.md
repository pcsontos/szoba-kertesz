...
| 07. | **A+B fázis:** `customers` tábla + `queryCustomers`, beszélgetés-perzisztencia (`threads` + `messages`), negyedik DB-szerep, thread-API, `--thread`, webes thread-lista és megosztható URL | **C fázis koncepciója azóta megépült** (2026-09-03-i orchestrátor-agent kör — lásd alább a "Hol tart a terv" táblázat utáni szakaszt); ami belőle kimaradt: UI-handover, `flow-test` mérőeszköz, router-mód. **D fázis** (voice miniapp) továbbra is nyitott |
...
A 07. alkalomban a C fázis azért maradt ki, mert önmagában nagyobb volt, mint az A+B együtt, a kódvezetés maga jelölte ki kiszállási pontnak az A+B végét, és a 08–09. alkalom nem épített rá. A döntés indoklása a `docs/superpowers/specs/2026-08-22-ora-07-perzisztencia-design.md` 1. döntésénél, a lezárás bizonyítéka a `docs/ora-07-zaro-ellenorzes.md`-ben.

**A C fázis koncepciója egy önálló, kurzus-alkalomhoz nem kötött körben (2026-09-03) megépült**: az orchestrátor-agent (`askOrchestrator` — harmadik agent, sosem válaszol saját szóval, egyetlen route-toolt hív) és a package-agent (`askPackageAgent` — negyedik agent, a csomag-építő, `validatePackage`/`savePackage`/`cancelPackage`) a flow-lockos "delegate" topológiával (`findLastFlowSignal`), a projekt eredeti architektúra-invariánsait (egy agent = prompt + tools + loop, egy agent lehet másik agent toolja, a szétválasztás a DB-jog szintjén él) követve. A terv: `docs/superpowers/plans/2026-09-03-orchestrator-agent.md`, a döntési spec: `docs/superpowers/specs/2026-09-03-orchestrator-agent-design.md`.

Amit ez a kör **szándékosan nem** épített meg a kurzus eredeti C fázisából, a spec 4. döntése szerint, külön körnek hagyva: **látható UI-handover** (chip, ami mutatja, melyik agent beszél épp), **`flow-test` mérőeszköz** (LLM-as-user forgatókönyvek, automatikus értékelés — a 08. alkalom `autotest`-jének mintájára önálló kör), és **router-mód** (`ORCHESTRATION_MODE` env-kapcsoló off/router/delegate között) — a projekt konvenciója kifejezetten elveti a felesleges feature flag-eket, a flow-lockos "delegate" topológia az egyetlen, ami megépült, nincs runtime-váltható alternatíva. **A D fázis** (voice miniapp) továbbra is teljesen nyitott — nem a kurzus zárultával szűnt meg, csak nem került sorra.
