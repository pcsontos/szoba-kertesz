...
5. **Egyetlen írási út.** A katalógust kizárólag az `upsertProduct` tool módosíthatja (szigorú Zod a határon, paraméterezett SQL, kódban rögzített oszlop-lista, latin névre kulcsolt upsert). Nyers write-SQL nincs a kódbázisban — a védelem kétszintű: tool-réteg ÉS Postgres-szerepkör.
6. **Átláthatóság beépítve.** Minden interakció JSONL-be naplózva (token usage-dzsel — ez a költségbecslés bizonyítékbázisa) + élő, színes Trace; `--show-prompt` a teljes prompt megjelenítéséhez, `--quiet` csak a konzol-felét némítja.
