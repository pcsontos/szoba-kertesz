...
`szoba-kertesz`: parancssori (CLI) AI agent, amely a természetes nyelvű kérdést SQL-re fordítja a növény-katalógus (`products`) felett, read-only lefuttatja, és természetes nyelvű választ ad. Önkiszolgáló analitika SQL-tudás nélkül.
...
- **FR1, Kérdezés:** `szoba-kertesz ask "<kérdés>"` egyszeri lekérdezés, és interaktív readline mód (amíg `exit`).
