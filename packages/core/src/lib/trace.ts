...
import type { ToolOutcome } from './tools/index.js';
...
export class Trace {
  ...
  toolStep(
    ...
    let result: unknown = outcome.resultSummary;
    try {
      result = JSON.parse(outcome.resultSummary);
    } catch {
    ...
    if (!outcome.ok) {
      this.line(c.red('  → hiba: ') + outcome.resultSummary);
    } else {
      this.line(
