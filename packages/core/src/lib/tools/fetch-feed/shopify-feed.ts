...
//
// Ez a fetch-feed tool MOTORJA, nem tool: sima kliens-kód, LLM nélkül, és az ADATBÁZISBA NEM
// ÍR — az egyetlen írási út az upsertProduct (tools/upsert-product/). A motor letölt + lapoz,
// kiszűri a nem-növényeket, kinyeri a biztonságosan kinyerhető mezőket, HUF-ra vált
// (USD 310, EUR 350), és dedupál latin név szerint.
//
...
/**
 * Amit NEM szabad növény-jelöltként visszaadni. A `product_type` kisbetűsítve érkezik ide.
 *
 * ⚠️ Az első blokk a kurzus angol listája — az a tropicalhome.hu feedjén CSAK az `Accessories`
 * és a `Soil` típust fogja meg. A második blokk a 2026-08-16-i élő mérés eredménye a valódi
 * feeden (307 termék): a `Kaspók` egymaga 66 tétel, azaz a jelöltek negyede kerámia kaspó
 * lenne — az ingest-agent kaspót írna a NÖVÉNY-katalógusba. A szűrést szándékosan nem az
 * LLM-re bízzuk: ez determinisztikus, mérhető és ingyen van.
 *
 * Új forrás bekötésekor (SOURCES) mérd meg a feed tényleges product_type-jait, és bővítsd.
 */
const NON_PLANT_TYPES = new Set([
  // a kurzus angol listája
  'accessory',
  'accessories',
  'planter',
  'consumable',
  ...
  'care',
  '',
  // mérve a tropicalhome.hu feedjén (2026-08-16)
  'kaspók',
  'kaspó',
  'pots & planters',
  'gift card',
...
const MAX_PAGES = 20;
/** Egy nem válaszoló feed nem akaszthatja meg a teljes agent-futást. */
...
/** A hálózati hívás injektálható — a teszt SOHA nem megy ki valódi HTTP-re. */
...
async function fetchPage(
  ...
  try {
    const res = await fetchImpl(url, {
      headers: { 'User-Agent': 'Mozilla/5.0 szoba-kertesz-ingest' },
...
async function fetchAllProducts(
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    const { products } = await fetchPage(url, fetchImpl);
...
function toCandidate(source: SourceConfig, p: ShopifyProduct): FeedCandidate {
  ...
  const compareAt =
    v.compare_at_price != null
      ? toHuf(v.compare_at_price, source.currency)
      : null;
...
export async function fetchFeedCandidates(
  ...
  for (const p of products) {
    ...
    if (!prev || richness(candidate) > richness(prev))
