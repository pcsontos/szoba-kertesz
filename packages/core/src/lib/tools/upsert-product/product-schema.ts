...
// Az LLM-output megbízhatatlan input → szigorú Zod a rendszer-határon (docs/konvenciók.md),
// fail-fast.
//
// ⚠️ AZ ENUMOK FORRÁSA a `docs/system-prompt.md` `<schema>` blokkja (és a vele egyező
// CLAUDE.md „Domain model: products" szakasza). Ha ott változik egy kötött szótár,
// ITT IS változnia kell — különben a query-agent és az ingest-agent más világot lát:
// a modell olyan értéket írna be, amire a promptja soha nem szűrne.
...
// A mezők és a hozzájuk tartozó snake_case oszlopok — a paraméterezett INSERT/UPDATE innen áll
// össze (egy helyen a sorrend, hogy az oszlop-lista és az értékek ne csússzanak el).
export const PRODUCT_COLUMNS: ReadonlyArray<
  readonly [keyof ProductInput, string]
> = [
  ['name', 'name'],
