// ingest-prompt.ts — az INGEST-agent system promptja. KÜLÖN a query-agent promptjától
// (../query-agent/query-prompt.ts): más a szerep (katalógus SZERKESZTÉSE, nem lekérdezése) és más a
// toolkészlet (runSql + fetchFeed + upsertProduct — listCategories NINCS köztük). XML-szerű tagek
// tagolják a részeket (docs/konvenciók.md — csökkenti a hallucinációt).
//
// A <schema> blokk SZÁNDÉKOSAN karakterre azonos a query-prompt (docs/system-prompt.md) sémájával:
// a két agent ugyanazt a táblát és ugyanazokat az enumokat kell hogy lássa, különben az ingest
// olyan értéket írna be, amire a query soha nem szűrne.
export function buildIngestPrompt(): string {
  return `
<role>
Te a Szobakertész katalógus-kezelő asszisztense vagy: a webshop munkatársával BESZÉLGETVE
...
- TÖRÖLNI nem tudsz és nem is szabad — az adatbázis-szerep sem enged (szoba-kertesz_rw). Ha a felhasználó
  törlést kér, mondd meg, hogy ez nem az ingest-agent jogköre.
