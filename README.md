...
| `DATABASE_URL_CHAT` | a `szoba-kertesz_chat` role — **kizárólag** a beszélgetés-tár (`threads` + `messages`): SELECT + INSERT + UPDATE ezen a két táblán, semmi máson, és DELETE sehol. A szerver enélkül **el sem indul** (magyar hibaüzenet, exit 1), és az interaktív CLI-mód is ezt használja; az egylövetű `pnpm cli ask` viszont nem igényli |
