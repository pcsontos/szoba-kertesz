...
- **The role switches a capability, not a prohibition** *(05. alkalom)*: as `admin` the `delegateToIngest` key goes **into** the query agent's toolset object; as `customer` the spread lands empty — the model does not even know the tool exists. This is stronger than a prohibition stated in the prompt: what is not offered cannot be called. The `role` deliberately lives on `AskAgentOptions`, **not** on `AskOptions`: the shared loop does not know which agent it runs, so it cannot know about roles either — it only sees the finished toolset. Two pinning specs (`query-agent.spec.ts`, `ingest-agent.spec.ts`) hold both toolsets in place.
